...
def main():
    args = parse_args()
    assert args.round_idx >= 2, "round_idx must be >= 2 (round 0 and round 1 are pre-computed)"
    ...
    prev_dir  = round_dir(args.round_idx - 1)
    input_data = {}
    for cul in culture_dict:
        if cul == args.culture: continue
        input_path = f"{prev_dir}/{model_tag}_{cul}_system.jsonl"
...
Output format MUST be exactly: \\boxed{{number}}"""
    ...
    # ---- Save ----
    output_dir = round_dir(args.round_idx)
    os.makedirs(output_dir, exist_ok=True)
    output_path = f"{output_dir}/{model_tag}_{args.culture}_system.jsonl"
